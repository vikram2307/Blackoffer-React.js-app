...
const EndYearChart = () => {
  ...
  const fetchData = async () => {
    try {
      // Fetch end year data
      const endYearsResponse = await fetch('http://localhost:8080/years');
      ...
      const endYearIntensitiesResponse = await fetch('http://localhost:8080/intensity/all/startyear');
      ...
      const likelihoodResponse = await fetch('http://localhost:8080/likelihoodbystartyear');
      ...
      const relevanceResponse = await fetch('http://localhost:8080/Relevancebystartyear');
  ...
  return (
    <div className="chart-container">
      <h2>Start Year Intensity, Likelihood, and Relevance Chart</h2>
      <div className="end-year-container">
        <div className="chart">
          <canvas ref={chartRef} id="end-year-chart" />
        </div>
        <div className="end-year-filter">
          <label htmlFor="endYearFilter">Select Start Year:</label>
          <select
            id="endYearFilter"
